 ...
 * Uses REPLACE conflict strategy to update existing entries automatically.
 */
suspend fun HealthDao.insertOverlayDataWithDeduplication(data: List<OverlayDataEntity>) {
    if (data.isEmpty()) return

    // Simply insert - the composite primary key (startTime, type) with OnConflictStrategy.REPLACE
